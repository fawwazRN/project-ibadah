...
// Klasemen dengan highlight tim santri
function MyStandingsTable({ rows, myTeamId }) {
  if (!rows.length)
      ...
      <table className="w-full min-w-[560px] text-sm text-left">
        ...
        <tbody>
          {rows.map((r) => (
            <tr
              key={r.team_id}
              className={`transition-colors hover:bg-white/[0.02] ${r.team_id === myTeamId ? "bg-brand/[0.06]" : ""} ${r.status === "relegated" ? "opacity-50" : ""}`}>
...
export default function LeagueStandingsPage() {
  ...
  useEffect(() => {
    // Hanya santri yang punya "timku" — staff tidak perlu
    if (profile?.role !== "santri") return;
    supabase
      .rpc("get_santri_league")
      .then(({ data }) => setMyTeamId(data?.team?.id ?? null));
  }, [profile?.role]);
      ...
      {tab === "team" && (
        <>
          ...
          <Card>
            ...
            />
            <MyStandingsTable rows={data.teamRows} myTeamId={myTeamId} />
          </Card>
        </>
      )}
        ...
        <>
          ...
          <Card>
            ...
            {data.players.filter(
              ...
              <EmptyState icon={Target} title="Belum ada statistik" />
            ) : (
